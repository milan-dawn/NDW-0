...
static int max_bufsize = 0;
static int sleep_wait_time = 0;
...
ndw_HandleAsyncMessage(ndw_Topic_T* topic, void* opaque)
{
    ...
    char* data = ndw_CreateTestJsonStringofNSize(topic, "Sample Message",
                            topic->sequence_number, 40, &data_size);
    data_size += 1;
...
on_poll_exit:
    ndw_ThreadExit();
    return args;
} // end method thread_getresreq_wait
...
test_initialize(int argc, char** argv)
{
    ...
    if (program_options->wait_time_seconds <= 0) {
        program_options->wait_time_seconds = 1;
    }
    ...
    max_msg = program_options->max_msgs;
    max_bufsize = program_options->bytes_size;
    sleep_wait_time = program_options->wait_time_seconds;
