...
void*
thread_publisher(void *arg)
{
    printf("Publisher Thread ID<%lu>\n", pthread_self());
    ...
    NDW_LOGX("Publisher Args: num_topics<%d> num_msgs<%d> msg_size<%d> delay_us<%d>\n",
                num_topics, args->num_msgs, args->msg_size, args->delay_us);

    NDW_LOGX("Total Number of Active Topics to Publish to is = %d\n", num_publications);
    ...
    for (int i = 0; i < num_topics; i++) {
        ...
        if (a_topic->Publish_Enabled) {
            if (NULL == a_topic->topic) {
                NDW_LOGERR("*** ERROR: ndw_Topic_T* pointer NOT set for <%s>\n", a_topic->TopicName);
                args->ret_code = -1;
                return args;
            }

            if (NULL == a_topic->function_publish_data.function) {
                NDW_LOGERR("AppTopic_T has NULL function_publish for Topic<%s>\n", a_topic->TopicName);
                args->ret_code = -1;
                return args;
            }

            a_topic->topic->sequence_number = 0;
        }
    }
    ...
    for (int i = 0; i < args->num_msgs; i++)
    {
        ...
        {
            AppTopic_T* a_topic = &Topics[j];
            ndw_Topic_T* topic = a_topic->topic;
            if (a_topic->Disabled || (! a_topic->Publish_Enabled) || NDW_ISNULLCHARPTR(topic->pub_key))
                continue;
            ...
            if (0 == (topic->sequence_number % print_frequency_modulo)) {
                NDW_LOG(">>> Published Message: sequence_number<%ld> data <%s> "
                          "data_size <%d>\n", topic->sequence_number, data, data_size);
            }

            free(data);
            a_topic->msgs_published_count += 1;
...
on_publisher_exit:
    ...
    args->ret_code = 0;
    return args;
} // end method thread_publisher

INT_T
subscribe()
{
    NDW_LOGX("BEGIN: Subscribing...\n");
    for (int i = 0; i < num_topics; i++) {
        AppTopic_T* a_topic = &Topics[i];
        ndw_Topic_T* topic = a_topic->topic;

        if (a_topic->Disabled || (! a_topic->Subscribe_Enabled) || NDW_ISNULLCHARPTR(topic->sub_key))
            continue;

        if (NULL == a_topic->function_subscribe_data.function) {
            NDW_LOGX("*** ERROR: subscribe_data.function NOT set for <%s %s, %s>\n",
                a_topic->Domain, a_topic->Connection, a_topic->TopicName);
            return -1;
        }

        NDW_LOGX("Subscribing to <%s %s, %s> using function<%s>\n",
                a_topic->Domain, a_topic->Connection, a_topic->TopicName, a_topic->function_subscribe_data.function_name);

        INT_T ret_code = a_topic->function_subscribe_data.function(topic);
        if (0 != ret_code) {
            NDW_LOGERR("*** ERROR: Subscription failed with return_code<%d> for %s\n", ret_code, topic->debug_desc);
            return ret_code;
        }
    }
    NDW_LOGX("END: Subscribing...\n");
...
void*
thread_subscriber_poll(void* arg)
{
    printf("Subscriber Poll Thread ID<%lu>\n", pthread_self());

    poll_subscriber_args *args = (poll_subscriber_args *) arg;
    if (args->timeout_sec <= 0) {
        args->timeout_sec = 6;
    }

    if (num_poll_subscriptions <= 0) {
        NDW_LOGERR("NOTE: There are no polling subscriptions enabled\n");
        args->ret_code = 0;
        return args;
    }

    int n_poll_subscriptions = 0;
    for (int i = 0; i < num_topics; i++)
    {
        AppTopic_T* a_topic = &Topics[i];
        ndw_Topic_T* topic = a_topic->topic;
        ...
        if (NULL == a_topic->function_subscribe_data.function) {
            NDW_LOGERR("*** ERROR: There is NO subscription function set for <%s>\n", a_topic->TopicName);
            args->ret_code = -2;
            return args;
        }

        if (NULL == a_topic->function_poll_data.function) {
            NDW_LOGERR("*** ERROR: Poll activity flag set, but there is NO poll function set for <%s>\n", a_topic->TopicName);
            args->ret_code = -3;
            return args;
        }

        if (NULL == a_topic->function_commit_data.function) {
            NDW_LOGERR("*** ERROR: Poll activity flag set, but there is NO commit function set for <%s>\n", a_topic->TopicName);
            args->ret_code = -4;
            return args;
        }

        if (NULL == a_topic->topic) {
            NDW_LOGERR("*** ERROR: ndw_Topic_T* Pointer not set <%s>\n", a_topic->TopicName);
            args->ret_code = -5;
            return args;
        }

        ++n_poll_subscriptions;
    }

    if (0 == n_poll_subscriptions) {
        NDW_LOGERR("*** WARNING: There is no Subscriptions to poll: n_poll_subscriptions<%d> num_poll_subscriptions<%d>\n",
                    n_poll_subscriptions, num_poll_subscriptions);
        args->ret_code = 0;
        return args;
    }
    ...
    while (1)
    {
        ...
        {
            AppTopic_T* a_topic = &Topics[i];
            if (a_topic->Disabled || (!a_topic->is_poll_activity))
                continue;

            ndw_Topic_T* topic = a_topic->topic;

            int num_items = a_topic->function_poll_data.function(topic, (LONG_T) a_topic->poll_timeout_us);

            if (1 == num_items) {
                a_topic->msgs_received_count += 1;
                at_least_one_received = 1;
                atomic_fetch_add(&total_poll_received, 1);
                char* msg = (char*) topic->last_msg_received;
                int msg_size = topic->last_msg_received_size;

                if (0 == (topic->total_received_msgs % print_frequency_modulo)) {
                    NDW_LOGX("<<< Received POLL Message: [%s, %ld] msg<%s> msg_size<%d> ON %s\n",
                                a_topic->TopicName, topic->total_received_msgs, msg, msg_size, topic->debug_desc);
                }

                INT_T commit_code = a_topic->function_commit_data.function(topic);
                if (0 != commit_code) {
                    NDW_LOGX("*** ERROR: Failed to commit messgage! commit_code<%d> msg<%s> msg_size<%d> on %s\n",
                        commit_code, msg, msg_size, topic->debug_desc);
                    goto on_poll_exit;
                }
            }
            else if (0 == num_items) {
...
test_initialize(int argc, char** argv)
{
    ...
    fprintf(stdout, "Publisher Thread Args: num_msgs<%d> msg_size<%d> delay_us<%d>\n",
            pub_arg->num_msgs, pub_arg->msg_size, pub_arg->delay_us);
...
#if 1
/*
 * Only for testing of loading JSON files.
 */
int
test_test_initialize(int argc, char** argv)
{
    ...
    fprintf(stdout, "==> test_initialize() completed. num_topics<%d> num_publications<%d> "
            "num_subscriptions<%d> num_poll_subscriptions<%d>\n",
            num_topics, num_publications, num_subscriptions, num_poll_subscriptions);
    ...
    if (num_publications > 0) {
        fprintf(stdout, "Creating Publisher Thread...\n");
    }

    if (num_poll_subscriptions > 0) {
        fprintf(stdout, "Creating Subscriber Poll Thread...\n");
    }
    ...
    if (num_publications > 0) {
        fprintf(stdout, "Join on Publisher Thread...\n");
    }

    if (num_poll_subscriptions > 0) {
        fprintf(stdout, "Join on Subscriber Poll Thread...\n");
    }
    ...
    return 0;
} // end method test_test_initialize
